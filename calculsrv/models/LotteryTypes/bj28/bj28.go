...
//注意, 由于BJ28的玩法特殊性 的特殊性, 原本api的开奖号码是21个数字 ,但是只计算 0-5 , 6-11, 12 - 17 的和值的个位数
//也就是说 开奖号码是3个 个位数字
//由于api服务器使用的是同一的类 所以最后还是由本类来处理
type BJ28 struct {
	id            int    //彩票ID
	gameTag       string //游戏名字
...
//初始化彩票
func Init(lif gb.LotteryInfo) (*BJ28, error) {
	ltry := &BJ28{}
...
//更新彩票信息
func (o *BJ28) updataInfo(newestRecord gb.LtryRecordByNewest) {
	//保存API获取的数据结构
	...
	//由于BJ28的特殊性 ,开奖号码在这一步进行计算
	//0 - 5 , 6 - 11 , 12 - 17
	var oc []int
	sum1 := newestRecord.OpenCode[0] + newestRecord.OpenCode[1] + newestRecord.OpenCode[2] + newestRecord.OpenCode[3] + newestRecord.OpenCode[4] + newestRecord.OpenCode[5]
	oc = append(oc, sum1%10)

	sum2 := newestRecord.OpenCode[6] + newestRecord.OpenCode[7] + newestRecord.OpenCode[8] + newestRecord.OpenCode[9] + newestRecord.OpenCode[10] + newestRecord.OpenCode[11]
	oc = append(oc, sum2%10)

	sum3 := newestRecord.OpenCode[12] + newestRecord.OpenCode[13] + newestRecord.OpenCode[14] + newestRecord.OpenCode[15] + newestRecord.OpenCode[16] + newestRecord.OpenCode[17]
	oc = append(oc, sum3%10)

	o.openCode = oc
	//当期开采号码String形式
	o.openCodeString = strconv.Itoa(oc[0]) + "," + strconv.Itoa(oc[1]) + "," + strconv.Itoa(oc[2])
...
//开采
func (o *BJ28) StartLottery(newestRecord gb.LtryRecordByNewest) {
	beego.Info("--- 开彩啦 : ", o.gameTag, " -------------------------\n")
	//检查是否掉期
...
func (o *BJ28) CheckLostExpect(gameTag string, newestRecord gb.LtryRecordByNewest, nextExpect int) {
	newestExpect := newestRecord.CurrentExpect
	//如果 最新获取的期数大于下棋期数,开始进入补全程序
	if newestExpect > nextExpect {
		beego.Error("发现掉期 开始进补全 : ", gameTag, " 最新期数为 : ", newestExpect, " 当前下期期数为 : ", nextExpect, "\n")
		...
		for _, v := range lostExpectInfo {
			...
			//补开采
			//注意这里bj28 开采号码要计算一次(bj28 特殊)
			tmp := strings.Replace(v.Opencode, "+", ",", -1)
			tmpOpenCode := utils.PaserOpenCodeToArray(tmp)

			var oc []int
			sum1 := tmpOpenCode[0] + tmpOpenCode[1] + tmpOpenCode[2] + tmpOpenCode[3] + tmpOpenCode[4] + tmpOpenCode[5]
			oc = append(oc, sum1%10)

			sum2 := tmpOpenCode[6] + tmpOpenCode[7] + tmpOpenCode[8] + tmpOpenCode[9] + tmpOpenCode[10] + tmpOpenCode[11]
			oc = append(oc, sum2%10)

			sum3 := tmpOpenCode[12] + tmpOpenCode[13] + tmpOpenCode[14] + tmpOpenCode[15] + tmpOpenCode[16] + tmpOpenCode[17]
			oc = append(oc, sum3%10)

			//当期开采号码String形式
			strOpencode := strconv.Itoa(oc[0]) + "," + strconv.Itoa(oc[1]) + "," + strconv.Itoa(oc[2])
			o.SettlementOrders(orders, strOpencode)

			//组数据发送给GS
			record := gb.LotteryRecord{}
			record.Expect = tmpExpect
			record.GameName = gameTag
			record.OpenCode = strOpencode
...
func (o *BJ28) updateGsLtryRecord() error {
	hist, err := apimgr.Instance().GetLtryHist(o.gameTag)
	if err != nil {
		return err
	}

	//由于BJ28玩法的特殊性,这里要将得到的历史记录全部转换为3位数的结果号码
	for i := 0; i < len(hist); i++ {
		tmp := utils.PaserOpenCodeToArray(hist[i].OpenCode)

		sum1 := tmp[0] + tmp[1] + tmp[2] + tmp[3] + tmp[4] + tmp[5]

		sum2 := tmp[6] + tmp[7] + tmp[8] + tmp[9] + tmp[10] + tmp[11]

		sum3 := tmp[12] + tmp[13] + tmp[14] + tmp[15] + tmp[16] + tmp[17]

		hist[i].OpenCode = strconv.Itoa(sum1%10) + "," + strconv.Itoa(sum2%10) + "," + strconv.Itoa(sum3%10)
	}
...
//更新GameServer的彩票信息
func (o *BJ28) updataGsLotteryInfo() {
	//组消息
...
//解析投注
func (o *BJ28) AnalyticalBetting(bettingInfo gb.MsgBettingInfo) int {
...
func (o *BJ28) CheckUnsettledOrderRecord() {
	//1. 找出这个菜种所有未结算订单
...
//得到下棋开彩时间
func (o *BJ28) GetNextOpenTime() time.Time {
	return o.nextOpenTime
}

//得到下期请求时间
func (o *BJ28) GetNextReqTime() time.Time {
	return o.nextRequestTime
}

//得到当前期数
func (o *BJ28) GetCurrentExpect() int {
	return o.currentExpect
}

//得到父类名字
func (o *BJ28) GetParentName() string {
	return o.parentName
}

//更新彩票信息(这个函数用于后台有彩票数据更新的情况,这时要更新彩票信息,并通知Gamesrv)
func (o *BJ28) UpdateLtryInfo(li gb.LotteryInfo) bool {
...
//更新彩票设置(赔率)
func (o *BJ28) UpdateLtrySet() bool {
	o.UpdateSettings()
...
//得到彩票名字
func (o BJ28) GetGameName() string {
	return o.gameTag
}
