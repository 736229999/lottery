...
//初始化彩票
func Init(lif gb.LotteryInfo) (*PK10, error) {
	ltry := &PK10{}
	...
	if ltry.gameTag == gb.PK10_BeiJing {
		var err error
		ltry.newestRecord, err = apimgr.Instance().GetRecordByNewest(lif.GameTag)
		if err != nil {
			return nil, err
		}
	} else if ltry.gameTag == gb.PK10_F { //如果是急速pk10需要从 独立彩票服务器去获取历史记录,如果有其他急速pk10菜种加入,就要在这里加入判断
		var err error
		ltry.newestRecord, err = ind.GetRecordByNew(lif.GameTag)
		if err != nil {
			return nil, err
		}
	}
...
//更新彩票信息
func (o *PK10) updataInfo(newestRecord gb.LtryRecordByNewest) {
	//保存API获取的数据结构
	...
	//下期请求时间
	o.nextRequestTime = newestRecord.NextOpenTime.Add(1 * time.Second)
	//beego.Debug("下期请求时间")
	...
	//下期截至下注时间
	if o.gameTag == gb.PK10_BeiJing {
		d, _ := time.ParseDuration("-60s")
		o.nextClosingBetTime = o.nextOpenTime.Add(d)
	} else if o.gameTag == gb.PK10_F {
		d, _ := time.ParseDuration("-1s")
		o.nextClosingBetTime = o.nextOpenTime.Add(d)
	} else {
		beego.Error("Not have this lottery type !")
	}
...
//开采
func (o *PK10) StartLottery(newestRecord gb.LtryRecordByNewest) {
...
func (o *PK10) CheckLostExpect(gameTag string, newestRecord gb.LtryRecordByNewest, nextExpect int) {
	...
	if newestExpect > nextExpect {
		...
		for _, v := range lostExpectInfo {
			...
			//beego.Debug(record)
			beego.Info("--- 掉期补全完成 Game Name : ", o.gameTag)
		}
	}
}
...
func (o *PK10) updateGsLtryRecord() error {
	//由于有急速pk10 所以这里要判断获取来源
	//等写完再来修改这里的结构,由于采用了json这种然后数据结构又在另外一个文件,这里就不可避免的要定义另外文件的数据结构,妈的我不想这么做,所以先复杂的写了,后面来改,草!~
	if o.gameTag == gb.PK10_BeiJing {
		...
		hist, err := ind.GetHist(o.gameTag)
		if err != nil {
			return err
		}
	...
	return errors.New("Not have this lottery type : " + o.gameTag)
}

//更新GameServer的彩票信息
func (o *PK10) updataGsLotteryInfo() {
	//组消息
...
//解析投注
func (o *PK10) AnalyticalBetting(bettingInfo gb.MsgBettingInfo) int {
...
func (o *PK10) CheckUnsettledOrderRecord() {
	//1. 找出这个菜种所有未结算订单
	...
	for expect, orders := range unsettleOrdersMap {
		...
		if o.currentExpect == expect {
			o.SettlementOrders(orders, o.openCodeString)
		} else if o.currentExpect > expect { //如果当前期数大于,订单的期数,那么要去数据库中找到对应期数的历史记录来进行结算
			//判断是普通pk10 还是急速pk10
			var ltryRecord gb.LotteryRecord
			var err error

			if o.gameTag == gb.PK10_BeiJing { //普通pk10去 api服务器获取
				ltryRecord, err = apimgr.Instance().GetLtryRecordByExpect(o.gameTag, expect)
				if err != nil {
					beego.Error("严重错误 :  根据彩种,和期数获取开采记录失败 请检查 ", err, " 彩种 : ", o.gameTag, " 期数 : ", expect)
					return
				}
			} else if o.gameTag == gb.PK10_F { //急速pk10去 indsrv 服务器获取
				ltryRecord, err = ind.GetRecordByExpect(o.gameTag, expect)
				if err != nil {
					beego.Error("严重错误 :  根据彩种,和期数获取开采记录失败 请检查 ", err, " 彩种 : ", o.gameTag, " 期数 : ", expect)
					return
				}
			...
			//3.结算同一期的这些订单
			o.SettlementOrders(orders, ltryRecord.OpenCode)
		}
	}
...
//得到下期请求时间
func (o *PK10) GetNextReqTime() time.Time {
...
//得到父类名字
func (o *PK10) GetParentName() string {
	return o.parentName
}

//更新彩票信息(这个函数用于后台有彩票数据更新的情况,这时要更新彩票信息,并通知Gamesrv)
func (o *PK10) UpdateLtryInfo(li gb.LotteryInfo) bool {
...
//更新彩票设置(赔率)
func (o *PK10) UpdateLtrySet() bool {
	o.UpdateSettings()
